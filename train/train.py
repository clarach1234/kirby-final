...
from transformations import (
    ...
    to_torch_tensors, pad_images_unet, random_cutout, add_skeleton_targets
)
from data_utils import SubsetRandomSampler, SubsetSequentialSampler, get_folds
from tracking import init_wandb, log_wandb_artifacts
...
def train(training_path, train_idx, test_idx, tracker=None):
    """ Trains the network.
    ...
    """
    topology_targets = (
        config.criterion == 'OfficialScoreLoss'
        or config.base_criterion == 'OfficialScoreLoss'
    )
    transforms_train = [
        pad_images_unet,
        random_hsv,
        ...
        random_cutout,
        *([add_skeleton_targets] if topology_targets else []),
        to_torch_tensors,
    ]
...
def train_sets(sets):
    ...
    for i, set_ in enumerate(sets):
        ...
        if config.model in ['CMRRWNet', 'CMRRWNetV2', 'RRWNet', 'RRWNetAll', 'RRUNet']:
    ...
    for instance in instances:
        ...
        tracker = init_wandb(
            config,
            fold=instance['i'],
            train_images=instance['train_imgs'],
            validation_images=instance['val_imgs'],
            run_dir=instance['train_path'],
        )
        try:
            train(
                instance['train_path'],
...
def multi_train():
    ...
    folds = get_folds(
        ...
        balanced=config.balanced_folds,
    )
    ...
    for p in processes:
        ...
        if p.exitcode != 0:
            raise RuntimeError(
                'Training subprocess {} failed with exit code {}'.format(
                    p.pid, p.exitcode
                )
            )
