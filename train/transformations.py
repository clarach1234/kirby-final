...
def random_affine(npimages):
    ...
    for i in range(len(npimages)):
        img = npimages[i]
        if i > 0:  # Ground truth, ROI mask, or another discrete target
            order = 0
        else:
            order = 3
...
def add_skeleton_targets(npimages):
    """Append fixed GT skeletons in model order [artery, vessel, vein]."""
    vessels = npimages[1]
    skeletons = np.stack(
        [skeletonize(vessels[..., i] > 0.5) for i in range(3)],
        axis=2,
    ).astype(np.float32)
    return list(npimages) + [skeletons]
