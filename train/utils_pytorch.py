...
class EarlyStopReduceLROnPlateau:
    ...
    def __init__(
        ...
        scheduler_path = self.filepath + '/scheduler.csv'
        if not os.path.exists(scheduler_path) or os.path.getsize(scheduler_path) == 0:
            save_to_csv([['epoch', 'new_lr']], scheduler_path)
