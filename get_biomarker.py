...
def extract_av_masks(av_img, threshold=127):
    """
    Args:
    ...
    # get_predictions.py saves soft RGB probabilities. Convert them to
    # booleans before applying the repository's colour-encoding rules.
    r_channel = av_img[:, :, 0] > threshold
    g_channel = av_img[:, :, 1] > threshold
    b_channel = av_img[:, :, 2] > threshold
...
def process_av_indicators(av_dir, disc_dir, output_dir, threshold=127):
    """
    计算动静脉7项指标并保存结果
    指标列表：
    1. CRAE - 视网膜中央动脉当量
    ...
    for fname in tqdm(av_files, desc="Calculating AV indicators"):
        try:
            ...
            artery_mask, vein_mask = extract_av_masks(av_img, threshold=threshold)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Extract the seven Task 3 biomarkers from A/V and optic-disc masks."
    )
    parser.add_argument('--av-dir', required=True,
                        help='Directory containing Task 2 RGB prediction PNGs')
    parser.add_argument('--disc-dir', required=True,
                        help='Directory containing optic-disc mask PNGs with matching names')
    parser.add_argument('--output-dir', required=True,
                        help='Directory in which Task 3 text files will be written')
    parser.add_argument('--threshold', type=int, default=127,
                        help='Threshold for the RGB probability PNGs (default: 127)')
